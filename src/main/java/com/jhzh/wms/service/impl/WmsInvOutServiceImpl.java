...
public class WmsInvOutServiceImpl implements WmsInvOutService {
	...
	public Result<?> wmsInvOut(JSONObject jsonObject) {
		try {
			...
			Result<Object> error = Result.error(CodeMsg.builder().build());
			//出库
			...
			if (EmptyUtils.isEmpty(ilsCellDtos)) {
				imesFeedBackService.wmsInvOutResultError(jsonObject);
				errorFlag=true;
				wmsInvOutDto.setStatuscode(14);
    ...
    @Override
    public Result<?> invOutqueueTask(JSONObject jsonObject) {
		//出库
	    queueTaskDao.insertQueueTask(QueueTaskDto.builder()
	    						.taskid((String) jsonObject.get("taskId"))
	    						.requestbody(jsonObject.toJSONString())
	    						.queuetype(2)
	    						.wipentityid((String) jsonObject.get("wipentityid"))
	    						.status(0)
	    						.build());
		return Result.success("");
    }
